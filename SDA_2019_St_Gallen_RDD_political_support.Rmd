...
However, I provide a power calculation around the point estimate of half the effect size estimated under the linear distance (jumo between the black lines). Note that the black lines might be biased by data far from the threshold and that particularly the one above the cutoff is quite far from the local polynomial fit. 
...
ols_abnb_robust <- robust.se(ols_abnb) # robust s.e.

stargazer(ols_abnb_robust, type = stargazer_output)
```
...
power_values %>% ggplot(aes(x = value, y = Power, color = Significance_level)) +
  ...
  labs(title = "Required sample per desired power",
       caption = "Effect size is half the estimated effect under the biased estimate, standard deviations are based on estimated standard deviation",
